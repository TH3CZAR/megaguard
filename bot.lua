...
http.TIMEOUT = 10
--------------------------------------------------------------------------------
sudo_users = {439620509,475082811}
bot_id = 475082811
day = 86400
ch_id = -100056843787
Token = '471747758:AAH8ki2NDxi0w0L0oAf810Zmozco9VHOKV0'
--------------------------------------------------------------------------------
function Run()
print('\27[93m>Developer:\27[39m'..' '..'@nikutintm')
end
--------------------------------------------------------------------------------
...
function ban(msg,chat,user)
  ...
  if priv(chat,user) then
reply_to(msg.chat_id_, msg.id_, 1,'▪شما نمیتوانید کاربر دارای مقام  را #بن کنید !', 1, 'md')
    else
  changeChatMemberStatus(chat, user, "Kicked")
  redis:sadd('banned'..chat,user)
  local t = '▪ کاربر '..user..' از گروه #سیک شد !'
...
function banall(msg,chat,user)
  ...
  if priv(chat,user) then
      reply_to(msg.chat_id_, msg.id_, 1,'▪ شما نمیتوانید کاربر دارای مقام را اخراج کنید !', 1, 'md')
    else
  changeChatMemberStatus(chat, user, "Kicked")
  redis:sadd('gbaned',user)
  local t = '▪ کاربر '..user..' از تمام گروه های من #سیک شد !'
  SendMetion(msg.chat_id_, user, msg.id_, t, 8, string.len(user))
  end
  end
--------------------------------------------------------------------------------
function mute(msg,chat,user)
  ...
  if priv(chat,user) then
      reply_to(msg.chat_id_, msg.id_, 1, '▪ شما نمیتوانید کاربر داری مقام را خفه کنید', 'md')
    else
  redis:sadd('mutes'..chat,user)
  local t = '▪ کاربر '..user..'  #خفه شد !'
  SendMetion(msg.chat_id_, user, msg.id_, t, 8, string.len(user))
  end
  end
--------------------------------------------------------------------------------
function unban(msg,chat,user)
  ...
  local t = '▪ کاربر '..user..' در گروه از حالت #سیک ازاد شد !'
  SendMetion(msg.chat_id_, user, msg.id_, t, 8, string.len(user))
  end
--------------------------------------------------------------------------------
function unbanall(msg,chat,user)
  ...
  local t = '▪ کاربر '..user..' از تمام گروهایی که سیک شده بود ازاد شد !'
  SendMetion(msg.chat_id_, user, msg.id_, t, 8, string.len(user))
  end
--------------------------------------------------------------------------------
function unmute(msg,chat,user)
  ...
  local t = '▪ کاربر '..user..' در گروه #ان_خفه شد !'
  SendMetion(msg.chat_id_, user, msg.id_, t, 8, string.len(user))
  end
--------------------------------------------------------------------------------
...
function warn(msg,chat,user)
  ...
  if type == "kick" then
    kick(msg,chat,user)
local text = '▪ کاربر '..user..' به دلیل دریافت اخطار بیش از حد #سیک شد !'
SendMetion(msg.chat_id_, user, msg.id_, text, 8, string.len(user))
    end
  if type == "ban" then
local text = '▪ کاربر '..user..' به دلیل دریافت اخطار بیش از حد #کیر شد !'
SendMetion(msg.chat_id_, user, msg.id_, text, 8, string.len(user))
changeChatMemberStatus(chat, user, "Kicked")
redis:sadd('banned'..chat,user)
  end
	if type == "mute" then
local text = '▪ کاربر '..user..' به دلیل دریافت اخطار بیش از حد #خفه شد !'
SendMetion(msg.chat_id_, user, msg.id_, text, 8, string.len(user))
...
addChatMember(text:match('join(.*)'), 335267337, 10)
 ...
 reply_to(msg.chat_id_,msg.id_,1,'▪ حلع '..text:match('join(.*)')..' ادت کردم.',1,'md')
...
-- lock join
    ...
    if is_owner(msg) then else
      if is_lock(msg,'pin') then
        if msg.content_.ID == 'MessagePinMessage' then
      reply_to(msg.chat_id_, msg.id_, 1, ' حاجی قفل پین فعالع \n ',1, 'md')
       unpin(msg.chat_id_)
...
--------------------------------------------------------------------------------
        if text == 'fwd' or text == 'فوروارد' and tonumber(msg.reply_to_message_id_) > 0 then
          function cb(a,b,c)
          local list = redis:smembers("UltraGrandgp")
          for k,v in pairs(list) do
...
reply_to(msg.chat_id_, msg.id_, 1, '▪ groups [`'..data.title_..'`] Haas been #Added !', 1, 'md')
else
redis:sadd('add', msg.chat_id_)
redis:set("groupc:"..msg.chat_id_,true)
reply_to(msg.chat_id_, msg.id_, 1, '▪ groups [`'..data.title_..'`] Haas Been #Now Added !', 1, 'md')
       end
end
--------------------------------------------------------------------------------
if text:match("^rem$") or text:match("^حذف$") and is_sudo(msg) then
if not redis:sismember('add', msg.chat_id_) then
reply_to(msg.chat_id_, msg.id_, 1, '▪ groups [`'..data.title_..'`] Haas Been #Not Added !', 1, 'md')
else
redis:srem('add', msg.chat_id_)
redis:del("groupc:"..msg.chat_id_,true)
reply_to(msg.chat_id_, msg.id_, 1, '▪ groups [`'..data.title_..'`] Haas Been #Now Removed !', 1, 'md')
end
end
--------------------------------------------------------------------------------
------شارژ", "charge")
if text:match('^charge (%d+)$') then 
 ...
 reply_to(msg.chat_id_, msg.id_, 1,'▪groups Haas Been #Charged '..text:match('charge (.*)')..' ',1,'md') 
end
--------------------------------------------------------------------------------
------تنظيم مالک", "setowner")
...
local text = '▪ User '..user..' Promoted To Owner...!'
SendMetion(msg.chat_id_, user, msg.id_, text, 8, string.len(user))
        end
--------------------------------------------------------------------------------
------حذف مالك", "deowner")
...
--------------------------------------------------------------------------------
        ...
        local text = '▪ User '..result.sender_user_id_..' Demoted From Owner...!'
SendMetion(msg.chat_id_, user, msg.id_, text, 8, string.len(user))
      end
        end
--------------------------------------------------------------------------------
...
local gps = redis:scard("UltraGrandgp")
	local users = redis:scard("usersbot")
					...
					reply_to(msg.chat_id_, msg.id_, 1, "▪ امار ربات مگا گارد گرند :\n\n▪ کاربران : <code>"..users.."</code>\n▪ گروه ها : <code>"..gps.."</code>\n▪ آپتایم : "..upt.."", 1, 'html')
	end 
--------------------------------------------------------------------------------
...
local nr = [[
Nerkh Bot [megacyber](http://telegram.me/megayber)
1 Mah ≈> 5 Toman
3 Mah ≈> 7 Toman
5 Mah ≈> 10 Toman
Na mahdod => 15 Toman
[Admin](http://telegram.me/nikutintm)
[Report](http://telegram.me/supmegabot)
[Channel](http://telegram.me/megacyber)
]]
          function inline(arg,data)
          tdcli_function({
        ID = "SendInlineQueryResultMessage",
...
if priv(msg.chat_id_,result.sender_user_id_) then
      reply_to(msg.chat_id_, msg.id_, 1,'▪ شما نمیتوانید به کاربر دارای مقام #اخطار دهید !', 1, 'md')
    else
...
if text == 'report' or text == 'ریپورت' then
  ...
  if priv(msg.chat_id_,result.sender_user_id_) then
reply_to(msg.chat_id_, msg.id_, 1,'▪ شما نمیتوانید کاربر دارای (  , مقام را #ریپورت کنید !', 1, 'md')
else
